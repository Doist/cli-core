...
Without `refresh`, `token` prints whatever is stored — which, for OAuth accounts whose access token has expired, is a token that no longer works. Pass `refresh: { provider, lockPath, skewMs?, handshake? }` (`TokenRefreshOptions`, the `refreshAccessToken` options minus `store` / `ref` / `force`) to rotate an expiring token first so `export TOKEN="$(cli auth token)"` always hands scripts a usable one. The env-var guard runs before any refresh, so a refused print never touches the network. Fallback rules, shared with `status`:

- `AUTH_REFRESH_UNAVAILABLE` (no refresh token, store without `activeBundle` + `setBundle`, provider without `refreshToken`, nothing stored) → fall back to the plain stored read, so static-token accounts and empty stores behave exactly as before (`NOT_AUTHENTICATED` / `ACCOUNT_NOT_FOUND` included).
- `AUTH_REFRESH_TRANSIENT` → fall back to the stored token only while it is still valid (no tracked expiry, or expiry in the future). Once it has expired the error propagates — a retryable failure beats handing a script a dead token.
- `AUTH_REFRESH_EXPIRED` (refresh token rejected; re-login required) and `AUTH_STORE_READ_FAILED` propagate unchanged.
...
`refreshAccessToken({ store, provider, lockPath, skewMs?, force?, ref?, handshake? })` rotates the access token using the stored refresh token. Use **proactively** before each authenticated call (skew defaults to 60s) or **reactively** with `force: true` after a 401. Persists the rotated bundle via `persistBundle` _without_ `promoteDefault` so a background rotation can't re-pin account selection. `handshake` (default `{}`) is forwarded to `provider.refreshToken` for resolvers that need runtime context (e.g. a `--env`-derived base URL). It may also be a function `({ account }) => Record | Promise<Record>` (`RefreshHandshake`), called with the stored account read under the refresh lock, for per-account context such as a DCR `clientId` or a per-instance base URL that lives on the account record rather than in the token bundle. The function is only invoked when a rotation is actually going to be attempted.
