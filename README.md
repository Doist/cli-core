...
it('greets the active account', async () => {
    ...
    const program = createTestProgram((p) => attachStatusCommand(p))
...
The `createTestProgram` helper requires `commander` (already an optional peer-dep). These helpers import
`vitest`, so only import this subpath from test files.
