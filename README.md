...
// Single-user impl — one account, ref ignored on active/clear, list returns
// a one-element array, setDefault validates the ref or throws.
export const tokenStore: TokenStore<Account> = {
    async active(_ref?: AccountRef) {
    ...
    async setDefault(ref: AccountRef) {
        const snapshot = await this.active()
        if (!snapshot || (snapshot.account.id !== ref && snapshot.account.label !== ref)) {
...
The three account-touching attachers (`attachLogoutCommand` / `attachStatusCommand` / `attachTokenViewCommand`) always attach `--user <ref>` on their subcommand and always thread the parsed value to `store.active(ref)` / `store.clear(ref)`. Single-user stores see a ref they can either match against their one account or report as missing via `CliError('ACCOUNT_NOT_FOUND', …)`.
