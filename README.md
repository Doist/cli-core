...
Stores that target servers issuing refresh tokens may implement the optional `setBundle(account, bundle, options?)` method and return a populated `bundle` field from `active()`. `TokenBundle` carries `{ accessToken, refreshToken?, accessTokenExpiresAt?, refreshTokenExpiresAt? }`. Stores that omit `setBundle` continue to work — cli-core helpers (`persistBundle`) fall back to `set(account, bundle.accessToken)` and silently drop refresh state. `KeyringTokenStore` implements `setBundle` as a required override and routes the refresh token to a sibling keyring slot.

The `persistBundle({ store, account, bundle, promoteDefault? })` helper is the canonical write path — it prefers `setBundle` when available, falls back to `set` otherwise, and wraps non-`CliError` failures as `AUTH_STORE_WRITE_FAILED`. Login persistence (and a future `refreshAccessToken` helper) both use it.
