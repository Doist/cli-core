 ...
 * arrive here, which is what lets any CLI adopt the feature with one call.
 */
...
import { DEFAULT_SECRET_ENV_VARS } from './npm.js'
...
export function createExtensionManager(options: ExtensionManagerOptions): ExtensionManager {
    ...
    // Upper-cased once here, so the case-insensitive match in `npm.ts` needs
    // to normalize only the side it reads from the environment.
    const secretEnvVars = new Set(
        [...DEFAULT_SECRET_ENV_VARS, ...(options.secretEnvVars ?? [])].map((name) =>
            name.toUpperCase(),
        ),
    )
