...
/**
 * Variables removed before running npm, whichever CLI is hosting. A denylist
 * rather than an allowlist: npm legitimately needs a large and open-ended
 * slice of the environment (proxies, certificate bundles, registry
 * credentials, package-manager settings), so naming what must not travel is
 * both safer in practice and easier to keep correct than naming everything
 * that may. The host adds its own credential variables through
 * `ExtensionManagerOptions.secretEnvVars`.
 */
export const DEFAULT_SECRET_ENV_VARS: readonly string[] = ['GH_TOKEN', 'GITHUB_TOKEN']
 ...
 * an exact-key delete would leave it in place. `secretEnvVars` is expected to
 * hold upper-cased names already.
 */
function environmentWithoutSecrets(secretEnvVars: ReadonlySet<string>): NodeJS.ProcessEnv {
    return Object.fromEntries(
        Object.entries(process.env).filter(([name]) => !secretEnvVars.has(name.toUpperCase())),
    )
}
...
export async function installDependencies(
    dir: string,
    secretEnvVars: ReadonlySet<string>,
): Promise<void> {
    ...
    const result = await run(command, [...prefix, ...args], {
        cwd: dir,
        env: environmentWithoutSecrets(secretEnvVars),
    })
