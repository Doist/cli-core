...
import { DEFAULT_SECRET_ENV_VARS, installDependencies } from './npm.js'

/** What the manager would hand over: the defaults plus one host credential. */
const SECRETS: ReadonlySet<string> = new Set([...DEFAULT_SECRET_ENV_VARS, 'XX_HOST_TOKEN'])
...
describe.skipIf(process.platform === 'win32')('installDependencies', () => {
    ...
    it('does nothing when the extension has no package.json', async () => {
        ...
        await installDependencies(extensionDir, SECRETS)
    ...
    it('keeps the CLI’s own credentials away from lifecycle scripts', async () => {
        vi.stubEnv('XX_HOST_TOKEN', 'secret-host-token')
        ...
        expect(environment).not.toContain('secret-host-token')
    ...
    it('reports a missing npm as its own failure', async () => {
        ...
        await expect(installDependencies(extensionDir, SECRETS)).rejects.toMatchObject({
