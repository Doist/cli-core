 ...
 *
 * Everything in `src/extensions/` is host-agnostic: the CLI's name, its
 * directories, its version and its reserved command names all arrive through
 * `ExtensionManagerOptions`. Nothing here reads a host's config or knows its
 * name, which is what lets every CLI share one implementation.
 */
...
export type ExtensionManagerOptions = {
    ...
    /**
     * Environment variables kept away from npm lifecycle scripts when an
     * extension's dependencies are installed, on top of `GH_TOKEN` and
     * `GITHUB_TOKEN`. Name the host's own credential variables here, e.g.
     * `TODOIST_API_TOKEN`. Matched without regard to case.
     */
