 ...
 *
 * Validation lives in `schemas.ts`, reached through `loadSchemas` only when a
 * file is actually parsed: discovery runs on every invocation of the CLI, and
 * most of those invocations never touch a manifest.
 */
...
export async function pickAuthoredFields(value: unknown): Promise<AuthoredManifest | undefined> {
    return (await loadSchemas()).parseAuthoredManifest(value)
}
...
export async function readInstalledManifest(
    ...
    return (await loadSchemas()).parseInstalledManifest(value)
}
