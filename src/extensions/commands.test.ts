...
import { writeFakeGitRepo, writeFixtureExtension } from '../testing/extensions.js'
...
describe('registerExtensionGroup', () => {
    ...
    beforeEach(async () => {
        root = await realpath(await mkdtemp(join(tmpdir(), 'td-ext-commands-')))
    ...
    describe('create', () => {
        it('is offered without the host naming templates, backed by the shipped ones', async () => {
            const program = makeProgram(makeManager())
            const group = program.commands.find((command) => command.name() === 'extension')
            expect(group?.commands.map((command) => command.name())).toContain('create')

            const argv = ['node', 'td', 'extension', 'create', 'goals', '--template', 'bash']
            vi.spyOn(process, 'argv', 'get').mockReturnValue(argv)
            vi.spyOn(process, 'cwd').mockReturnValue(root)
            await program.parseAsync(argv)
            expect(await readFile(join(root, 'td-goals', 'td-extension.json'), 'utf8')).toContain(
                '"td": ">=',
            )
        })
...
describe('registerExtensionPassThrough', () => {
    ...
    beforeEach(async () => {
        root = await realpath(await mkdtemp(join(tmpdir(), 'td-ext-passthrough-')))
