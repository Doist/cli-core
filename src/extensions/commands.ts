...
import { createExtension, defaultTemplatesDir } from './create.js'
...
Examples:
  ...
  ${binName} extension remove goals`,
    ...
    extension
        ...
        .option('--json', 'Output as JSON')
        .action((name, createOptions) => createScaffold(manager, templatesDir, name, createOptions))
...
export type ExtensionCommandOptions = {
    /**
     * Where the scaffold templates live, one subdirectory per template.
     * Overrides the templates this package ships.
     */
    templatesDir?: string
