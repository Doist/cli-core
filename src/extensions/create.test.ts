...
import { createExtension, defaultTemplatesDir, listTemplates } from './create.js'

/** The templates this package actually ships, so the tests cover the real files. */
...
describe('the templates td ships', () => {
    ...
    it.each(['bash', 'node'])('scaffolds a complete %s extension', async (template) => {
        ...
        // The templates are written for any host, so nothing td-specific may
        // survive except through the placeholders: the executable has to call
        // back through the variables this host sets, not ones spelled out.
        const executable = await readFile(join(result.dir, 'td-goals'), 'utf8')
        expect(executable).toContain('TD_NODE')
        expect(executable).toContain('TD_PATH')
