...
import { authoredManifestFileName, manifestFileName, toDirName } from '../extensions/source.js'

export type ExtensionFixtureOptions = {
    /** Host binary name the extension belongs to. Defaults to `td`. */
    binName?: string
    /** Environment variable prefix the fixture reports. Defaults to `binName` upper-cased. */
    envPrefix?: string
 ...
 */
function nodeFixture(envPrefix: string): string {
    return `#!/usr/bin/env node
const args = process.argv.slice(2)
const env = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => key.startsWith('${envPrefix}_') || key.startsWith('XX_')),
)
...
function shFixture(envPrefix: string): string {
    return `#!/bin/sh
echo "args:$*"
echo "name:$${envPrefix}_EXTENSION_NAME"
exit 0
`
}
...
export async function writeFixtureExtension(
    ...
    const envPrefix = options.envPrefix ?? binName.toUpperCase()
    const dirName = toDirName(binName, name)
    const dir = join(parentDir, dirName)
    await mkdir(dir, { recursive: true })

    const executablePath = join(dir, dirName)
    const body = options.interpreter === 'sh' ? shFixture(envPrefix) : nodeFixture(envPrefix)
