 ...
 *
 * The answer has to match what commander will go on to dispatch, because a
 * host's lazy loader uses it to decide which module to import and whether to
 * start the early spinner. Scanning all of argv for the first recognised name
 * gets that wrong twice over: `td --progress-jsonl task today` loads the task
 * module while commander runs `today`, and with extensions installed `td
 * goals task list` would load the task module on its way to an extension that
 * never wanted it.
 *
 ...
 *
 * The default covers the two value-taking flags cli-core's own global-args
 * parser knows. `--progress-jsonl` declares its value optional, but commander
 * still takes a following non-flag token for it, so this has to as well. A
 * host with more such flags passes its own set, kept in step with its
 * `program.option(...)` calls.
 */
